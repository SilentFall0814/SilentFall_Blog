...
import com.silentfall.blog.exception.BlockedException;
...
public class BlockServiceImpl implements BlockService {
    ...
    public void checkIfBlocked(String fingerprint) {
        ...
        // 检查数据库
        Query query = new Query(Criteria.where("fingerprint").is(fingerprint));
        Visitors visitor = mongoTemplate.findOne(query, Visitors.class);
        if (visitor != null && visitor.getIsBlocked() == 1) {
            if (visitor.getExpiresAt() == null || visitor.getExpiresAt().isAfter(LocalDateTime.now())) {
                ...
                Query updateQuery = new Query(Criteria.where("id").is(visitor.getId()));
                Update update = new Update().set("isBlocked", 0).set("expiresAt", null);
                mongoTemplate.updateFirst(updateQuery, update, Visitors.class);
            }
        }
    }
    ...
    private void blockVisitor(String fingerprint) {
        ...
        if (visitor != null) {
            ...
            Update update = new Update()
                    .set("isBlocked", 1)
                    .set("expiresAt", LocalDateTime.now().plusDays(1));
            mongoTemplate.updateFirst(updateQuery, update, Visitors.class);

            // 更新Redis缓存
            String blockedKey = BLOCKED_KEY + fingerprint;
            redisTemplate.opsForValue().set(blockedKey, "1", 1, TimeUnit.DAYS);
            log.warn("封禁访客: fingerprint={}", fingerprint);
        }
    }
}
