...
import com.silentfall.blog.service.SaveLogAsyncService;
...
public class SaveLogAsyncServiceImpl implements SaveLogAsyncService {
    ...
    public void saveLogAsync(JoinPoint joinPoint, Object result,
        ...
        try {
            ...
            // 记录操作用户
            String adminIdLong = BaseContext.getCurrentId();
            if (adminIdLong != null) {
                operationLogs.setAdminId(adminIdLong);
            }

            // 获取目标ID,从SpEL表达式中解析
            if (!operationLog.targetId().isEmpty()) {
                String targetId = parseTargetId(joinPoint, operationLog.targetId());
                if (targetId != null) {
     ...
     * 解析目标ID（SpEL表达式）
     */
    private String parseTargetId(JoinPoint joinPoint, String targetIdExpression) {
        try {
