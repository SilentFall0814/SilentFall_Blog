...
public class MessageController {
     ...
     */
    @GetMapping
    public Result<List<MessageVO>> getMessageTree(@RequestParam(required = false) String visitorId) {
    ...
    @RateLimit(type = RateLimit.Type.IP, tokens = 5, burstCapacity = 8,
              timeWindow = 60, message = "操作过于频繁，请稍后再试")
    public Result<String> deleteMessage(@PathVariable String id, HttpServletRequest request) {
        String visitorId = visitorTokenService.resolveVisitorId(request);
