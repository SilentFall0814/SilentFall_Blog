package com.silentfall.blog.handler;
...
import com.silentfall.blog.exception.TokenException;
...
public class GlobalExceptionHandler {
    ...
    /**
     * 捕获控制器方法参数校验异常（@NotNull/@NotBlank 等用在 @RequestParam/@PathVariable 上时触发）
     * Spring 6+ 新增异常类型，不处理会被兜底捕获返回 500
     */
    @ExceptionHandler
    @ResponseStatus(HttpStatus.BAD_REQUEST)
    public Result exceptionHandler(HandlerMethodValidationException ex){
        // 提取所有参数校验错误信息
    ...
    /**
     * 数据库唯一约束/外键约束异常（MongoDB 重复键也会触发）
     */
    @ExceptionHandler
    @ResponseStatus(HttpStatus.BAD_REQUEST)
    public Result exceptionHandler(DataIntegrityViolationException ex){
        ...
        return Result.error("数据冲突，请检查是否存在重复值");
    }
