...
export default function LatestChatterCarousel({ chatters }: { chatters: any[] }) {
  const safeChatters = chatters && chatters.length > 0 ? chatters : [FALLBACK_CHATTER];
  const [currentIndex, setCurrentIndex] = useState(0);

  useEffect(() => {
    if (safeChatters.length <= 1) return;
    const timer = setInterval(() => {
      setCurrentIndex((prev) => (prev + 1) % safeChatters.length);
    }, 6000);
    return () => clearInterval(timer);
  }, [safeChatters.length]);

  const currentChatter = safeChatters[currentIndex] || FALLBACK_CHATTER;
  ...
  return (
    <div className="w-full h-full rounded-3xl bg-white/40 dark:bg-slate-800/50 backdrop-blur-md border border-white/40 dark:border-white/10 shadow-xl overflow-hidden relative group min-h-[220px] flex flex-col">
      ...
      <AnimatePresence mode="wait">
        ...
        >
          <img src={currentChatter.cover || FALLBACK_CHATTER.cover} className="w-full h-full object-cover opacity-80 dark:opacity-60 transition-transform duration-1000 group-hover:scale-105" alt="Chatter Cover" />
