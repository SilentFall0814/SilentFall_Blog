...
export default function MomentList({ moments, authorName, avatarUrl }: any) {
  ...
  const [deleteConfirmType, setDeleteConfirmType] = useState<'owner' | 'guest'>('owner');
  ...
  useEffect(() => { fetchGuestMoments(); }, []);

  // 🌟 新增：删除本地物理文件的辅助函数（带简单的引用检查）
  const deleteLocalFile = async (url: string, allMoments: any[]) => {
    if (!url || !url.startsWith('/uploads/')) return;

    // 🌟 引用检查：确保没有其他说说还在使用这个 URL
    const isUsed = allMoments.some(m => m.images && m.images.includes(url));

    if (isUsed) {
      console.log(`ℹ️ 跳过物理删除：文件 ${url} 仍被其他说说引用`);
      return;
    }
    ...
    try {
      ...
      const data = await res.json();
      console.log(`🗑️ 物理文件清理结果 (${url}):`, data.message);
    } catch (error) {
      console.error("❌ 物理文件清理失败:", error);
    }
  };
  ...
  const processedMoments = useMemo(() => {
    ...
    const pendingMoments = operations
      ...
      .map(op => ({
        ...
        _date: op.payload.date,
        _content: op.payload.content,
        _isHtml: false,
      }));
    ...
    let result = [...pendingMoments, ...baseMoments.map((m: any) => ({
      ...m,
      _type: 'owner' as const,
      _authorName: authorName,
      _avatarUrl: avatarUrl,
    ...
    return result;
  }, [moments, searchQuery, sortOrder, operations, guestMoments, authorName, avatarUrl]);
  ...
  const handleFileUpload = async (files: FileList | File[]) => {
    ...
    if (!isLocal && !picToken) { showToast("未配置图床 Token！", "error"); return; }
    ...
    try {
      ...
      for (let i = 0; i < files.length; i++) {
        ...
        else showToast(`第 ${i+1} 张上传失败: ${data.message || '未知错误'}`, "error");
      }
      if (newUrls.length > 0) {
  ...
  const executeDelete = async () => {
    ...
    try {
      ...
      if (deleteConfirmType === 'guest') {
        ...
        const data = await res.json();
        if (data.success) {
          showToast("🗑️ 访客说说已彻底删除", "success");
          fetchGuestMoments();
        } else {
          showToast(`删除失败: ${data.message}`, "error");
        }
      } else {
        const momentToDelete = moments.find((m: any) => m.id === deleteConfirmId);
        if (momentToDelete && momentToDelete.images) {
          const otherMoments = moments.filter((m: any) => m.id !== deleteConfirmId);
          for (const url of momentToDelete.images) {
            await deleteLocalFile(url, otherMoments);
          }
        }
  ...
  const handleDeleteClick = (moment: any) => {
    if (moment.isPending) {
      ...
      setDeleteConfirmType(moment._type === 'guest' ? 'guest' : 'owner');
    }
  };
    ...
    >
      ...
      <div className={`flex items-center gap-4 mb-8 pb-6 border-b border-slate-200/50 dark:border-slate-700/50 relative ${moment.isPending ? 'mt-4' : ''}`}>
        <div className="w-14 h-14 shrink-0 rounded-2xl overflow-hidden shadow-md border-2 border-white dark:border-slate-700">
          <img src={moment._avatarUrl || avatarUrl} alt="avatar" className="w-full h-full object-cover" />
        </div>
        <div className="flex flex-col">
          <div className="flex items-center gap-2">
            <h3 className="text-lg font-black text-[#576b95] dark:text-[#7f99cc] tracking-wide">{moment._authorName || authorName}</h3>
            {moment._type === 'guest' && (
          ...
          <div className="flex items-center gap-2 text-[11px] text-slate-400 font-bold mt-1"><Clock size={12} /> {timeAgo(moment._date || moment.date)}</div>
        </div>
      </div>
      {moment._isHtml ? (
        <div className="text-slate-800 dark:text-slate-200 text-[16px] leading-relaxed font-medium break-words prose prose-sm dark:prose-invert max-w-none [&_p]:mb-2 [&_h1]:text-lg [&_h2]:text-base [&_h3]:text-sm [&_ul]:list-disc [&_ol]:list-decimal [&_pre]:bg-slate-100 [&_pre]:dark:bg-slate-800 [&_pre]:rounded-lg [&_pre]:p-3 [&_code]:text-indigo-500" dangerouslySetInnerHTML={{ __html: moment._content }} />
      ) : (
        <p className="text-slate-800 dark:text-slate-200 text-[16px] leading-relaxed whitespace-pre-wrap font-medium break-words">{moment._content || moment.content}</p>
      )}
