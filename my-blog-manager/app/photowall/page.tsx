...
export default function PhotoWallPage() {
  ...
  const deleteLocalFile = async (url: string, albumsToCheck: Album[]) => {
    if (!url || !url.startsWith('/uploads/')) return;

    // 🌟 引用检查：确保没有其他相册或照片还在使用这个 URL
    const isUsed = albumsToCheck.some(album => 
      album.cover === url || album.photos.some(p => p.url === url)
    );

    if (isUsed) {
      console.log(`ℹ️ 跳过物理删除：文件 ${url} 仍被其他相册引用`);
      return;
    }
              ...
              <div className="flex gap-3">
                ...
                <button onClick={async () => {
                   ...
                   if(deleteModal.type === 'album') {
                     const albumToDelete = editableAlbums.find(a => a.id === deleteModal.id);
                     next = editableAlbums.filter(a => a.id !== deleteModal.id);
                     
                     if (albumToDelete) {
                       // 🌟 物理清理：删除封面和相册内所有照片（需检查其他相册是否还在引用）
                       await deleteLocalFile(albumToDelete.cover, next);
                       for (const photo of albumToDelete.photos) {
                         await deleteLocalFile(photo.url, next);
                       }
                     }
                   }
                   else {
                     ...
                     if(album) {
                        const photoToDelete = album.photos[deleteModal.photoIndex!];
                        album.photos = album.photos.filter((_, i) => i !== deleteModal.photoIndex);
                        next = [...editableAlbums];
                        setCurrentAlbum({...album});
                        
                        if (photoToDelete) {
                          // 🌟 物理清理：检查该照片是否还在其他地方使用
                          await deleteLocalFile(photoToDelete.url, next);
                        }
                     }
                   }
