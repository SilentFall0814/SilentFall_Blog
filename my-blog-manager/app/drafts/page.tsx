...
function DraftsContent() {
  ...
  const fetchDrafts = async () => {
    ...
    try {
      ...
      // 先获取正确的 blogPath
      const pathRes = await fetch(`http://127.0.0.1:${configData.api_port}/api/sync/config`);
      let currentBlogPath = "";
      if (pathRes.ok) {
        const pathData = await pathRes.json();
        currentBlogPath = pathData.blogPath || "";
        setBlogPath(currentBlogPath);
      }

      const res = await fetch(`http://127.0.0.1:${configData.api_port}/api/drafts/list`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ blog_path: currentBlogPath })
      });
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      const res = await fetch(`http://127.0.0.1:${configData.api_port}/api/drafts/delete`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ blog_path: blogPath, id })
      });
      ...
      <PageTransition>
        <main className="w-[90%] max-w-5xl mx-auto mt-28 relative z-10">
          ...
          <div className="mb-8 flex gap-4">
            ...
            <Link href="/editor?id=new&type=post" className="px-6 py-3 bg-pink-500 hover:bg-pink-600 text-white rounded-2xl font-black text-sm shadow-lg shadow-pink-500/30 transition-all active:scale-95 flex items-center gap-2">
              <Sparkles size={16} /> 新建文章草稿
            </Link>
          </div>
