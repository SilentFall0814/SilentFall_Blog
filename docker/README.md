# SilentFall Docker 部署指南

> ⚠️ **重要提醒**: 文档中的域名 `example.com`、`admin.example.com` 等仅为示例，请根据实际情况替换为你自己的域名。

本项目支持使用 Docker 进行快速部署，包含后端 Java 应用、MongoDB、Redis 和 Nginx 反向代理。
...
└── docker/
    ...
    │   ├── conf.d/
    │   │   └── SilentFall-backend.conf  # 后端代理配置
    │   └── sites-enabled/
    │       └── example.com      # 前端站点配置
    ├── mongo/
    │   └── init/           # MongoDB 初始化脚本
...
# MongoDB配置 (必须修改)
# ============================================
MONGODB_URI=mongodb://SilentFall:your_password_here@mongo:27017/SilentFall
...
MongoDB 无需手动建表，Spring Data MongoDB 会在应用启动时自动创建所需的集合。如需导入初始数据：

```bash
# 启动 MongoDB 容器
docker-compose up -d mongo

# 等待 MongoDB 启动完成，然后导入初始数据（如有）
docker exec -i SilentFall-mongo mongoimport --db SilentFall --collection user --jsonArray < docker/mongo/init/user.json
```
...
```bash
# 进入 Nginx 容器
docker exec -it SilentFall-nginx sh
...
docker-compose logs -f mongo       # MongoDB 日志
docker-compose logs -f redis       # Redis 日志

# 进入容器
docker exec -it SilentFall-backend sh
docker exec -it SilentFall-mongo mongosh
docker exec -it SilentFall-redis redis-cli
...
1. **数据库初始化**: MongoDB 无需手动建表，Spring Data MongoDB 会自动创建集合
...
# 检查 Nginx 配置
docker exec SilentFall-nginx nginx -t
```
