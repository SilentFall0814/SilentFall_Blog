...
import { MapPin, MessageSquare, Clock, Sparkles, Search, ArrowDownAZ, ArrowUpZA, ChevronLeft, ChevronRight, Ghost, PenLine } from 'lucide-react';
import MomentComments from '../../components/MomentComments';
import VisitorMomentEditor from '../../components/VisitorMomentEditor';
...
export default function MomentList({ moments, authorName, avatarUrl }: any) {
  ...
  const [guestMoments, setGuestMoments] = useState<GuestMoment[]>([]);
  ...
  const allItems = useMemo(() => {
    const ownerMoments = (moments || []).map((m: any) => ({
      ...m,
      _type: 'owner' as const,
    ...
    const guestItems = guestMoments.map((g: GuestMoment) => ({
      id: g.id,
      _type: 'guest' as const,
      _authorName: g.author,
      _avatarUrl: g.avatar,
      _date: g.created_at,
      _content: g.content,
      _isHtml: true,
    ...
    return [...ownerMoments, ...guestItems];
  }, [moments, guestMoments, authorName, avatarUrl]);
  ...
  const processedMoments = useMemo(() => {
    ...
    if (searchQuery.trim()) {
      ...
      result = result.filter(m =>
        (m._content || '').toLowerCase().includes(query) ||
        (m.location || '').toLowerCase().includes(query) ||
        (m._authorName || '').toLowerCase().includes(query)
      );
    }

    result.sort((a, b) => {
      const timeA = new Date(a._date).getTime();
      const timeB = new Date(b._date).getTime();
  ...
  const renderImages = (images: string[]) => {
    ...
    if (count === 1) {
      return (
        <div className="mt-4 md:mt-8 flex justify-start sm:justify-center w-full">
          <div onClick={() => setLightbox({ images, index: 0 })} className="max-w-full sm:max-w-[280px] overflow-hidden rounded-xl md:rounded-2xl border border-slate-200/50 dark:border-white/10 shadow-lg md:shadow-xl cursor-zoom-in group">
    ...
    return (
      <div className="w-full flex justify-start sm:justify-center mt-4 md:mt-8">
        <div className="grid gap-1.5 md:gap-2 sm:mx-auto moment-images-grid" style={{ gridTemplateColumns: `repeat(${columns}, 1fr)`, width: '100%', maxWidth: maxWidth }}>
    ...
    >
      <div className="flex items-center gap-3 md:gap-4 mb-4 md:mb-8 pb-4 md:pb-6 border-b border-slate-200/50 dark:border-slate-700/50 relative">
        <div className="w-10 h-10 md:w-14 md:h-14 shrink-0 rounded-xl md:rounded-2xl overflow-hidden shadow-sm md:shadow-md border-2 border-white dark:border-slate-700">
          <img src={moment._avatarUrl} alt="avatar" className="w-full h-full object-cover" />
        </div>
        <div className="flex flex-col">
          <div className="flex items-center gap-2">
            <h3 className="text-base md:text-lg font-black text-[#576b95] dark:text-[#7f99cc] tracking-wide">{moment._authorName}</h3>
            {moment._type === 'guest' && (
              <span className="px-2 py-0.5 rounded-full text-[9px] font-bold bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border border-emerald-500/20">访客</span>
            )}
          </div>
          <div className="flex items-center gap-1.5 md:gap-2 text-[10px] md:text-[11px] text-slate-400 font-bold mt-0.5 md:mt-1"><Clock size={10} className="md:w-3 md:h-3" /> {timeAgo(moment._date)}</div>
        </div>
      </div>

      {moment._isHtml ? (
        <div className="text-slate-800 dark:text-slate-200 text-[14px] md:text-[16px] leading-relaxed font-medium break-words prose prose-sm dark:prose-invert max-w-none [&_p]:mb-2 [&_h1]:text-lg [&_h2]:text-base [&_h3]:text-sm [&_ul]:list-disc [&_ol]:list-decimal [&_pre]:bg-slate-100 [&_pre]:dark:bg-slate-800 [&_pre]:rounded-lg [&_pre]:p-3 [&_code]:text-indigo-500" dangerouslySetInnerHTML={{ __html: moment._content }} />
      ) : (
        <p className="text-slate-800 dark:text-slate-200 text-[14px] md:text-[16px] leading-relaxed whitespace-pre-wrap font-medium break-words">{moment._content}</p>
      )}
    ...
    <div className="w-[95%] md:w-[90%] max-w-6xl mx-auto py-6 md:py-10 mt-24 md:mt-28 relative z-10 flex-1 flex flex-col min-h-[85vh]">
      ...
      <div className="mb-10 md:mb-16 flex flex-col items-center gap-5 md:gap-8">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-4 md:px-6 py-1.5 md:py-2.5 bg-indigo-500 text-white rounded-xl md:rounded-2xl text-[10px] md:text-xs font-black shadow-lg shadow-indigo-500/30 hover:bg-indigo-600 transition-all hover:scale-105 active:scale-95"
          >
            <PenLine size={12} className="md:w-3.5 md:h-3.5" /> 我也说一句
          </button>
        </div>
      </div>
      ...
      <LayoutGroup>
        {processedMoments.length > 0 ? (
          <div className="flex flex-col md:flex-row gap-5 md:gap-8 pb-32 w-full items-start moments-columns">
