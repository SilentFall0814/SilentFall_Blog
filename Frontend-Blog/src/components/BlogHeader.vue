...
// 移动端音乐面板展开状态
const mobileMusicVisible = ref(false)
...
// 移动端：切换音乐列表面板
const toggleMobileMusic = () => {
  mobileMusicVisible.value = !mobileMusicVisible.value
}
  ...
  <header class="site-header" :class="{ scrolled, dark: isDark }">
    <div class="header-inner">
      <div class="header-left">
        <router-link to="/" class="site-title">SilentFall's Blog</router-link>
    ...
    <nav v-show="mobileNavVisible" class="nav-mobile">
      ...
      <!-- 移动端音乐播放入口 -->
      <a
        v-if="blogStore.musics.length"
        class="nav-mobile-link"
        @click="toggleMobileMusic"
      >
        <i class="iconfont icon-yinle" /> 音乐
        <!-- 当前播放歌曲信息 + 播放控件 -->
        <span v-if="currentTrack" class="mobile-music-info">
          <button
            class="mobile-music-btn"
          ...
          <button class="mobile-music-btn" @click.stop="nextTrack" title="下一首">
            <i class="iconfont icon-next" />
          </button>
          <span class="mobile-music-title">{{ currentTrack.title }}</span>
        </span>
      </a>
      <!-- 移动端音乐列表面板 -->
      <div v-show="mobileMusicVisible" class="mobile-music-panel">
        <div class="mobile-music-panel-header">
          <span><i class="iconfont icon-yinle" /> 播放列表</span>
          <span class="mobile-music-panel-count"
            >{{ blogStore.musics.length }} 首</span
          >
        </div>
        <ul class="mobile-music-list">
          <li
            v-for="(m, idx) in blogStore.musics"
            :key="m.id"
            class="mobile-music-item"
            :class="{ active: idx === musicIndex }"
            @click="playTrack(idx)"
          >
            <img
              v-if="m.coverImage"
              :src="m.coverImage"
              class="mobile-music-cover"
            />
            <div class="mobile-music-meta">
              <span class="mobile-music-name">{{ m.title }}</span>
              <span class="mobile-music-artist">{{ m.artist }}</span>
            </div>
            <i
              v-if="idx === musicIndex && isPlaying"
              class="iconfont icon-yinle playing-icon"
...
.dark .mobile-music-panel {
  background: rgba(0, 0, 0, 0.15);
  border-bottom-color: #333;
}
.mobile-music-panel-header {
  display: flex;
  justify-content: space-between;
  align-items: center;
  padding: 8px 24px;
...
.dark .mobile-music-panel-header {
  color: #b0b0b0;
}
.mobile-music-panel-header .iconfont {
  margin-right: 4px;
}
.mobile-music-panel-count {
  font-size: 11px;
  color: #909399;
  font-weight: 400;
}
...
.mobile-music-item:hover {
  background: rgba(0, 0, 0, 0.03);
}
.dark .mobile-music-item:hover {
  background: rgba(255, 255, 255, 0.05);
}
.mobile-music-item.active {
  background: rgba(0, 0, 0, 0.04);
}
.dark .mobile-music-item.active {
  background: rgba(255, 255, 255, 0.08);
}
.mobile-music-item.active .mobile-music-name {
  color: #000;
  font-weight: 600;
}
.dark .mobile-music-item.active .mobile-music-name {
  color: #fff;
}
.mobile-music-cover {
  width: 36px;
